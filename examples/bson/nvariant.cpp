...
int main()
{
	...
	std::vector<std::uint8_t> encoded =  bobl::bson::encode(value);
    auto begin = encoded.data();
	auto end = begin + encoded.size();
	auto variant =  bobl::bson::decode<boost::variant<Simple, Extended, bobl::UseTypeName>>(begin, end);
	boost::get<Extended>(variant);

	begin = encoded.data();
	auto doc = bobl::bson::decode<Doc>(begin, end);
	return 0;
}
